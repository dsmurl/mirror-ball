...
- `mirror-ball-creator` — for preview and deploy (Pulumi preview/up), building and pushing the API image to ECR, syncing the site to S3, and invalidating CloudFront.
- `mirror-ball-destroyer` — for destroying the stack via a manual workflow.
...
- destroy.yml — manual (workflow_dispatch): assume `mirror-ball-destroyer` and run Pulumi destroy on selected stack.
